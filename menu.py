...
class Menu:
        ...
        for key, value in EDITOR_DATA.items():
            if value['menu']:
                if not value['menu'] in self.menu_surfaces:
                    ...
                    self.menu_surfaces[value['menu']].append(
                        (key, load(value['menu_surf'])))
    ...
    def create_buttons(self):
        ...
        size = 180
        margin = 6
        top_left = (WINDOW_WIDTH - size - margin,
        ...
        self.enemy_button_rect = generic_button_rect.move(
            self.rect.width / 2, self.rect.height / 2).inflate(-button_margin, -button_margin)

        # create the buttons
        self.buttons = pygame.sprite.Group()
        Button(self.tile_button_rect, self.buttons,
               self.menu_surfaces['terrain'])
        Button(self.coin_button_rect, self.buttons, self.menu_surfaces['coin'])
        Button(self.enemy_button_rect, self.buttons,
               self.menu_surfaces['enemy'])
        Button(self.palm_button_rect, self.buttons,
               self.menu_surfaces['palm fg'], self.menu_surfaces['palm bg'])
